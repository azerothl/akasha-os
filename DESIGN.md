...
- Fira Sans 400 / 500 / 600 for lede, running copy, and paper-plate captions
- Hydrogen reserved for the Install / Download winding key
- Honesty as one cyan swipe plus paper plates; inner pages continue in cyan, not hydrogen
...
- **Body** (400, 1.0625rem, line-height 1.55): Running copy, history, transcripts. Measure 70ch. Plate body is 0.98rem void on paper. Honesty `dd` is 0.98rem / 28ch.
- **Label** (600, 0.82rem, tracking 0.08em, uppercase): Paper-plate captions (Host, NVIDIA, Boot). Canvas orbit IDs are Archivo Narrow 600 in signal. Footer is 0.92rem mute.

### Named Rules
**The Two Voices Rule.** Archivo Narrow speaks the machine (rails, swipe, titles, winding key, continue). Fira Sans speaks the plates and the log (lede, captions, copy). Do not swap them.
...
Sky fold is a 2fr / 1fr grid (machine | honesty) with the winding key spanning the bottom row. The canvas is fixed full-viewport behind the shell. Honesty is a void column: lede, swipe, paper plates, repo link. Sky footer is hidden.
...
**The Cut-Face Rule.** Paper plates, grants, ticks, and chapter bands are separated by void gaps. Do not wrap them in a card grid.
...
### Named Rules
**The Field Not Card Rule.** Do not lift content on drop shadows to imply cards. The live orrery and tonal wells (paper on void, 8% signal mix under `pre`) carry depth.

## Shapes

Every control is a flush rectangle. No corner radius ships. Paper plates are square sheets. The winding key is a full-width hydrogen rail (on plate-main it bleeds to the inset edges). The mark is a circular A inscribed in cyan rings on a void square — identity geometry, not a UI radius. The orrery itself is ellipses, rods, and globes; that geometry stays on the canvas.
...
### Cards / Containers
Landing honesty uses paper plates: void type on paper, Fira caption + body, 2u padding, 2u gaps. Inner pages do not reuse those sheets for grants or chapters — those are cut-face stacks on void. Linked ticks go to signal on hover. `pre` is signal type on an 8% signal-into-void well, Fira 500, 0.95rem. Ordered procedures use Fira body with Archivo Narrow 700 leading-zero counters in signal.
...
- **Do** put HOST / NVIDIA rec. / NOT BOOTABLE in one cyan swipe, then paper plates.
...
- **Don't** put Archivo Narrow on plate captions, or Fira on the winding key.
