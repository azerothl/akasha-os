...
3. When a delegated child finishes, the runtime injects `[child-done]` and marks the step Done. Then `plan.update_step` (status) or `plan.replace_tree` if the plan must change. Do not wait blindly.
