            ...
            .show(ctx, |ui| {
                ...
                overflow_scroll(ui, "agent_detail_body", |ui| {
                    ...
                    if let Some(id) = active {
                        ...
                        {
                            let fr = self.prefs.language == "fr";
                            egui::CollapsingHeader::new(t.agents_policies)
                                .default_open(false)
                                .show(ui, |ui| {
                                    ui.horizontal_wrapped(|ui| {
                                    ...
                                    if self.agent_ui.policies.contains_key(&id) {
                                        ...
                                        ui.horizontal(|ui| {
                                            ...
                                            ui.radio_value(
                                                &mut net_deny,
                                                false,
                                                if fr { "Autorisé" } else { "Allowed" },
                                            );
                                            ui.radio_value(
                                                &mut net_deny,
                                                true,
                                                if fr { "Refusé" } else { "Denied" },
                                            );
                                        });
                                        ui.checkbox(
                                            &mut fs_write,
                                        ...
                                        ui.horizontal(|ui| {
                                            if crate::ui_primitives::danger_confirm_button(
                                                ...
                                                let policy =
                                                    crate::agent_ui_state::PolicyEditDraft {
                                                        net_deny,
                                                        fs_write,
                                                ...
                                                self.toasts.push_success(if fr {
                                                    "Politique envoyée (live si l'agent tourne, sinon au redémarrage)"
                                                        .to_string()
                                                } else {
                                                    "Policy sent (live if running, else on restart)"
                                                        .to_string()
                                                });
                                            }
                                        });
