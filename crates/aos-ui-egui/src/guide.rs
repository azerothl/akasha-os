...
const EN: GuideStrings = GuideStrings {
    ...
    chat_opt_documents_body: "On a research-style question, choose Reply for a short answer in chat, or Prepare a document for a structured file under /downloads/documents/. While it runs you see Researching… with Stop; when ready, Open shows the document in a clamped overlay. Generated documents are added to the Library (More → Library) and indexed for chat context.",
    chat_opt_documents_example: "\"What's the state of agentic apps?\" → Prepare a document → Researching… → Ready → Open; find it again under Library.",
...
const FR: GuideStrings = GuideStrings {
    ...
    chat_opt_documents_body: "Sur une question de recherche, choisissez Répondre pour une courte réponse dans le chat, ou Préparer un document pour un fichier structuré sous /downloads/documents/. Pendant la préparation : Recherche en cours… avec Arrêter ; une fois prêt, Ouvrir affiche le document dans une fenêtre calée au viewport. Les documents générés sont ajoutés à la Bibliothèque (Plus → Bibliothèque) et indexés pour le chat.",
    chat_opt_documents_example: "« Quel est l'état de l'art des apps agentic ? » → Préparer un document → Recherche en cours… → Prêt → Ouvrir ; retrouvez-le dans Bibliothèque.",
