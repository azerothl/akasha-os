...
const EN: GuideStrings = GuideStrings {
    ...
    create_intro: "Create turns text (and optional images) into stills or short clips. Controls stack: each option narrows or steers the diffusion run.",
    create_prompt_title: "Prompt",
    create_prompt_body: "Describes the subject, style, lighting, and composition. The image model reads this every run. A vague prompt gives generic results; concrete nouns and adjectives steer layout and mood.",
    create_prompt_example: "« A teal mug on a wooden desk, morning light, soft shadow » → a single centered object with warm highlights.",
    create_reference_title: "Reference image (paperclip)",
    create_reference_body: "Attach a still via the paperclip next to the prompt. The engine uses img2img: your photo becomes the starting latent. Strength (~0.75 default) controls how much the output may drift from the reference — lower keeps pose and colors; higher follows the prompt more.",
    create_reference_example: "Attach a portrait photo + prompt « wearing a red scarf » → same face, new accessory; lower strength keeps the original pose.",
    create_inpaint_title: "Inpaint (fix a region)",
    create_inpaint_body: "After a result appears, enable inpaint on the composition canvas, paint a mask over the area to change, then Generate. Only masked pixels are re-diffused; the rest of the image stays fixed.",
    create_inpaint_example: "Paint over the sky only + prompt « sunset clouds » → horizon and foreground unchanged, new sky.",
    create_video_title: "Video mode & duration",
    create_video_body: "Switch Image / Video at the top of the form. Video uses Wan/LTX packs and sets clip length in seconds (frame count is derived). Prompt should describe motion, not only a still scene.",
    create_video_example: "Video + 3 s + « steam rising from the mug » → a short loop of subtle motion.",
    create_format_title: "Width, height & profile",
    create_format_body: "Pixel size sets output resolution (and VRAM use). Profile (fast / balanced / quality) picks step count and sampler presets for the selected pack. Larger sizes and quality profile slow generation but add detail.",
    create_format_example: "512×512 fast → quick draft; 1024×1024 quality → sharper final still.",
    create_generate_title: "Generate",
    create_generate_body: "Runs diffusion with the current prompt, pack, size, and any reference / mask / video options. Optional enrichment (collapsed sections) rewrites the prompt via the chat LLM before image gen. Requires an installed image or video pack.",
    create_generate_example: "Pick a pack, write a prompt, click Generate → preview appears on the right; history keeps prior results.",
...
const FR: GuideStrings = GuideStrings {
    ...
    create_intro: "Créer transforme du texte (et des images optionnelles) en images ou courts clips. Les options se cumulent : chaque réglage oriente la diffusion.",
    create_prompt_title: "Prompt",
    create_prompt_body: "Décrit le sujet, le style, la lumière et la composition. Le modèle image le lit à chaque génération. Un prompt vague donne un résultat générique ; des détails concrets orientent la mise en page.",
    create_prompt_example: "« Une tasse bleue sur un bureau en bois, lumière du matin, ombre douce » → un objet centré avec reflets chauds.",
    create_reference_title: "Image de référence (trombone)",
    create_reference_body: "Joignez une image via le trombone à côté du prompt. Le moteur fait de l'img2img : votre photo sert de point de départ. La force (~0,75 par défaut) contrôle l'écart autorisé — plus bas garde pose et couleurs ; plus haut suit davantage le prompt.",
    create_reference_example: "Portrait joint + prompt « avec une écharpe rouge » → même visage, nouvel accessoire ; force basse garde la pose.",
    create_inpaint_title: "Inpaint (corriger une zone)",
    create_inpaint_body: "Après un résultat, activez l'inpaint sur le canevas de composition, peignez un masque sur la zone à modifier, puis Générer. Seuls les pixels masqués sont re-diffusés ; le reste reste fixe.",
    create_inpaint_example: "Masque sur le ciel seulement + prompt « nuages au coucher du soleil » → horizon inchangé, nouveau ciel.",
    create_video_title: "Mode vidéo et durée",
    create_video_body: "Basculez Image / Vidéo en haut du formulaire. La vidéo utilise les packs Wan/LTX et la durée en secondes (les images sont dérivées). Décrivez le mouvement, pas seulement une scène fixe.",
    create_video_example: "Vidéo + 3 s + « vapeur qui monte de la tasse » → courte boucle de mouvement subtil.",
    create_format_title: "Largeur, hauteur et profil",
    create_format_body: "La taille en pixels fixe la résolution (et la VRAM). Le profil (fast / balanced / quality) choisit les presets d'étapes et d'échantillonnage. Plus grand et « quality » ralentissent mais ajoutent du détail.",
    create_format_example: "512×512 fast → brouillon rapide ; 1024×1024 quality → image finale plus nette.",
    create_generate_title: "Générer",
    create_generate_body: "Lance la diffusion avec le prompt, pack, taille et options référence / masque / vidéo courants. L'enrichissement (sections repliées) réécrit le prompt via le LLM de chat avant la génération. Nécessite un pack image ou vidéo installé.",
    create_generate_example: "Choisissez un pack, écrivez un prompt, Générer → l'aperçu apparaît à droite ; l'historique garde les résultats.",
