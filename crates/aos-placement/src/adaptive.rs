...
impl Default for ThermalController {
    fn default() -> Self {
        Self { policy: ThermalPolicy::Balanced, soft_limit_c: 78.0, hard_limit_c: 88.0 }
    }
}
...
impl BackendRegistry {
    pub fn for_hardware(hw: &HardwareProfile, allow_experimental: bool) -> Self {
        let mut backends = vec![BackendDescriptor {
            ...
            quantizations: vec![Quantization::F16, Quantization::Q8, Quantization::Q6, Quantization::Q5, Quantization::Q4],
            gemm: true,
            gemv: true,
            batching: true,
            kv_cache: true,
...
impl AdaptivePlanner {
    pub fn new(hardware: HardwareProfile, options: PlannerOptions) -> Self {
        ...
        Self { hardware, registry, options }
    }
    ...
    pub fn select(
        ...
        let preferred = forced_backend.filter(|kind| {
            self.registry.get(*kind).is_some_and(|backend| backend.executable)
        }).or_else(|| {
            ...
            self.registry
                .backends
                .iter()
                .find(|b| b.kind != BackendKind::Cpu && b.executable && model_supports_backend(model, b.kind))
                .map(|b| b.kind)
        });
        let backend = preferred.filter(|b| self.registry.get(*b).is_some()).unwrap_or(BackendKind::Cpu);
        let descriptor = self.registry.get(backend).expect("CPU backend is always registered");
        ...
        let placement = if thermal_policy == ThermalPolicy::Quiet {
            PlacementProfile::MemorySaver
        } else if backend == BackendKind::Cpu {
        ...
        let fallback = self.registry.backends.iter().filter_map(|b| {
            (b.kind != backend && b.kind != BackendKind::Lan).then_some(b.kind)
        }).collect();
        let reason = format!(
            "{}: {} / {} / {:?} ({} tokens)",
            if forced_backend.is_some() { "forcé" } else { "automatique" },
            descriptor.name,
            quantization.as_str(),
            workload,
            kv_tokens
        ...
        InferencePlan {
            ...
            quantization,
            placement,
            kv_cache: if backend == BackendKind::Cpu { KvCacheType::F16 } else { KvCacheType::Q8_0 },
            kv_tokens,
            speculative,
            thermal_policy,
    ...
    pub fn compare_profiles(&self, model: &ModelDesc, kv_tokens: u32) -> Vec<InferencePlan> {
        [PlacementProfile::Latency, PlacementProfile::Balanced, PlacementProfile::MemorySaver, PlacementProfile::CpuOnly]
            .into_iter()
            .map(|profile| self.select(model, profile, WorkloadKind::Chat, kv_tokens, None))
            .collect()
    }

    fn choose_quantization(&self, model: &ModelDesc, backend: &BackendDescriptor) -> Quantization {
        let declared = model.quantization.format.as_deref().and_then(Quantization::parse);
        let candidate = declared.unwrap_or_else(|| Quantization::parse(&model.name).unwrap_or(Quantization::Q4));
        if backend.quantizations.contains(&candidate) && candidate.quality() >= self.options.min_quality {
            candidate
        } else {
            backend.quantizations.iter().copied()
                .filter(|q| q.quality() >= self.options.min_quality)
                // Prefer the smallest representation that satisfies the
                // quality gate; a lower quantization must not silently turn
                // into an unbounded F16 allocation.
                .min_by(|a, b| a.quality().partial_cmp(&b.quality()).unwrap())
...
fn model_supports_backend(model: &ModelDesc, backend: BackendKind) -> bool {
    ...
    let names = model
        ...
        .collect::<Vec<_>>();
    match backend {
        BackendKind::Cpu | BackendKind::Cuda | BackendKind::Metal => {
            names.iter().any(|name| name == "llamacpp" || name == "llama.cpp")
        }
...
mod tests {
    ...
    fn cpu_profile_never_selects_gpu() {
        let planner = AdaptivePlanner::new(HardwareProfile::cpu_only_laptop(), PlannerOptions::default());
        let plan = planner.select(&model_3b(), PlacementProfile::Balanced, WorkloadKind::Chat, 2048, None);
    ...
    fn experimental_adapters_are_opt_in() {
        ...
        hw.npu = Some(crate::NpuCapabilities { name: "test".into(), memory_bytes: 1, int8: true, experimental: true });
        assert!(AdaptivePlanner::new(hw.clone(), PlannerOptions::default()).registry.get(BackendKind::Npu).is_none());
        let mut options = PlannerOptions::default();
        options.allow_experimental = true;
        assert!(AdaptivePlanner::new(hw, options).registry.get(BackendKind::Npu).is_some());
    }
    ...
    fn metal_and_thermal_policy_are_selected_from_profile() {
        ...
        let plan = planner.select(&model_3b(), PlacementProfile::Latency, WorkloadKind::Chat, 2048, None);
    ...
    #[test]
    fn low_quality_quantization_falls_back_to_q4() {
        let mut model = model_3b();
        model.quantization.format = Some("Q2_K".into());
        let planner = AdaptivePlanner::new(HardwareProfile::reference_v1(), PlannerOptions::default());
        let plan = planner.select(&model, PlacementProfile::Balanced, WorkloadKind::Chat, 2048, None);
        assert_eq!(plan.quantization, Quantization::Q4);
    }
}
