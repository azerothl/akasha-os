...
- Document fichier demandé par l'utilisateur (présentation, rapport, livrable, « fais-moi un document ») : `files.generate` sous `/downloads/` (md de préférence) — le runtime l'ajoute à la **Bibliothèque**. **Pas** `notes.create` comme livrable final. Si `files.generate` n'est pas au catalogue, dis-le clairement. Tu peux utiliser des notes en brouillon avant de générer le fichier.
