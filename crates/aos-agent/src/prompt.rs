...
- Notes (carnet interne Notes) : `notes.create` avec titre + **outline court**, puis `notes.update` **section par section** (≤ ~1200 caractères de `content` par appel). Ne mets jamais un guide entier dans un seul JSON. Les notes servent à la réflexion / handoff entre agents — **pas** au livrable utilisateur.
- Document fichier demandé par l'utilisateur (présentation, rapport, livrable, « fais-moi un document ») : `files.generate` sous `/downloads/documents/` (md de préférence) — le runtime l'ajoute à la **Bibliothèque**. **Pas** `notes.create` comme livrable final. Si `files.generate` n'est pas au catalogue, dis-le clairement. Tu peux utiliser des notes en brouillon avant de générer le fichier.
