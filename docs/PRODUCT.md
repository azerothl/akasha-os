...
Akasha OS is an agent-native operating system: agents, models, tools, and memory are first-class system services with explicit capabilities, audit, and policy. Preview 0.17.2 is an installable host app on Windows/Linux (NVIDIA optional via CPU path in the same zip) and macOS Apple Silicon (unsigned), not a bootable OS image. A seL4 bare-metal track is separate.
...
- Preview 0.17.2: Windows/Linux x64; macOS Apple Silicon (unsigned); NVIDIA GPU recommended on Win/Linux; CPU-only path in the same Win/Linux artefact; ~8 GB disk recommended. No Intel Mac.
