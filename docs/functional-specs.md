...
> Changes v0.8.0: Preview 0.8.0 — E16 image/TTS (packs download engines); E17 unified CPU/GPU artefact; F-MDL-04 Providers; F-MOD-01 uninstall; E15 widget pack.
...
| F-UI-10 | Installed modules with `ui.mode=declarative_ui` get a host-rendered human surface (closed widget vocabulary: form, table, stats, charts, select/radio/checkbox, image/audio); no HTML/JS webview in Preview | Should |
...
- `docs/FEATURES.md` — shipped Preview 0.8.0 catalogue  
- (future) `adr/` — Architecture Decision Records  

---

## 12. Preview 0.8.0 coverage (host)
...
|----------|----------------|
| F-BOOT-01–05 | Hardware probe + model setup + offline chat; CPU backend in the same zip (E17) |
...
| F-MDL-01–09 | Local llama.cpp + optional remote; routing local_only / balanced / remote_only |
| F-MDL-04 | **Shipped Preview 0.8.0** — Providers tab (OpenAI-compat cloud + loopback) |
...
| F-MOD-01 | **Shipped Preview 0.8.0** — uninstall UI, revoke caps, refuse bundled |
| F-UI-10 / E15 | Shipped 0.7.0; **0.8.0** expands closed kinds (`select`/`radio`/`checkbox`/`textarea`/`bar_chart`/`image`/`audio`, typed `form`) |
| E16 | **Shipped Preview 0.8.0** — `media.image.generate` / `media.audio.generate` |
