...
- Vidéo : après complétion du sidecar Gemma (SHA-256 vérifié), `sd.cpp` charge le pack LTX et une génération IPC 256×256, 9 frames, 1 étape produit `engine=sdcpp`, WebM 111629 octets avec signature EBML `1A 45 DF A3`. Le défaut vidéo est désormais WebM ; MP4 reste explicitement non supporté par le binaire livré (AK-001, AK-009).
...
- Conformité desktop : **3/4**. L’application est une UI egui native et ne dépend pas d’un navigateur ; le bouton audio ouvre toutefois un chemin système externe, sans état de lecture dans le fil.
...
- Tests checkout : 375 tests UI, 261 agent + 11 worker + 1 daemon, 29 modèle et 172 platform réussis (849 au total ; platform/modèle sans CUDA) ; `cargo check --workspace` réussi.
...
Évolutions restantes : ajouter une lecture intégrée et un contrôle de durée/codecs dans l’UI ; le conteneur WebM de recette est valide.
...
Le client de recette, les garde-fous vidéo, les libellés audio, le smoke-test UI, les tokens d’état et les signatures catalogue ont été corrigés ou déployés selon les tickets ci-dessus.

## À vérifier avant validation complète

À poursuivre : contrôle focus clavier/DPI et lecteurs multimédias intégrés, contrôle durée/codecs WebM, débat multiagent avec mesure du nettoyage, mémoire/RAG avancée, outils/permissions (révocation), intégrations externes et récupération après erreur sous charge. Le démarrage devancé UI↔bus, la vision de base et le viewport minimum sont désormais couverts par AK-013 à AK-015.

## Axes d’amélioration priorisés

1. **P1 — Vidéo** : conserver WebM comme format natif du binaire livré, ajouter lecture intégrée et contrôle durée/codecs ; le garde-fou anti-faux-succès est en place.
...
4. **P2 — Audio** : ajouter lecture/pause/durée dans le fil, avec ouverture système en action secondaire.
