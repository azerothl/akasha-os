...
> **Preview honesty:** wgpu `scene3d` is an **edit view**, not RenderService beauty. Beauty = stub/CPU in-tree; Blender = mock unless opt-in **GPL Renderer Pack** + binary. Compose = model-proposed SceneIntent + bounded solver, with EN/FR keyword templates as fallback (not full LLM Asset/Pose/Composition planners). Neural mesh = stub default; weights opt-in. Marketplace = offline packs only. This is **not** a claim of full MVP §140–§145. Tester-facing summary: [FEATURES.md §4c](FEATURES.md#4c-illustration-studio-experimental-module).
