...
- **Room mode** (slice 1, proto + persistence only): `ChatSessionMode::Room` extends the same
  `ChatSession` with in-app salon members and optional `speaker_id` on session transcript lines.
  No UI, no Telegram/Discord channel, no conductor runtime yet — distinct from messaging channels
