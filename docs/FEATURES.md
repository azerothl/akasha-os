...
Catalogue of **shipped** Preview features on the Windows/Linux/macOS host.
...
- **Illustration Studio** (experimental): local catalogue module **0.7.23** in the Preview zip on **Windows, Linux, and macOS** — DeclUI scene editor; wgpu edit ≠ beauty; stub/CPU beauty; Blender mock unless opt-in GPL Renderer Pack ([§4c](#4c-illustration-studio-experimental-module))
...
- **Vision chat**: `InferRequest.images` carries local PNG/JPEG paths; catalog mmproj sidecars for Gemma / Qwen vision profiles; `generate_with_images` on the llama path; UI paperclip / image chips; gated when the session model lacks vision
- **Canvas tools**: scene-aware agent tools; `set_style` stroke colors live in the WASM module; draw fan-out fixes
- **macOS Apple Silicon**: Preview zip on GitHub Releases (Metal + CPU `aos-modeld`); mill third download band; unsigned build — Gatekeeper note on Mac tooltip only
...
**Chat canvas (vector strokes, live)** — session-scoped drawing surface (human + agents). Humans: Select (move / delete / align / rotate), Pan + scroll zoom, pen, line, spline (click-to-place Bézier), path (filled silhouette), rect, ellipse, text labels (click + inline edit, rotation); per-op opacity, dashed strokes, linear gradient fills; named layers (hide / lock / opacity); grid and 0.01 snap; export PNG, SVG, or JSON; **import JSON sidecar** (round-trip). Agents: bundled WASM `canvas.*` (`stroke` / `path` / `rect` / `ellipse` / `text` / `move` / `delete` / `align` / `rotate` / `layer_*` / `get` / `export`). A CPU-only global vector validator checks geometry/topology every three mutations and before completion; vision remains optional. Flood-fill (`canvas.fill`) is raster-only on export — not a human toolbar tool. Document: `var/sessions/<id>/canvas.json`. Distinct from Image Studio and `/image`.
...
| Assets | Native GLB import plus three curated Poly Haven CC0 models downloaded on demand with source, licence and dimensions; downloaded and imported assets can be dragged into the viewport; general marketplace fetch remains disabled |
...
| `harness.run` | Opt-in allowlisted spawn of local `codex` / `claude` / `grok` CLIs (prompt only, act-gate even in autonomous chat) — details [harness.md](harness.md) |
| External harness backend | Agents Advanced → **Runtime** = Codex / Claude / Grok: task workers run CLI turns under `aos-agent-worker`; **Steer** continues the session (`exec resume` / `-c`), **Pause** cancels the in-flight turn, **Kill** stops the worker tree — [harness.md](harness.md) |
...
| Site | [azerothl.github.io/akasha-os](https://azerothl.github.io/akasha-os/) — cloud chamber, EN/FR |
