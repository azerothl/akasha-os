...
- **Studio Illustration** (expérimental) : module catalogue local **0.7.23** dans le zip Preview sur **Windows, Linux et macOS** — éditeur de scène DeclUI ; édition wgpu ≠ beauté ; beauté stub/CPU ; Blender mock sauf Renderer Pack GPL opt-in ([§4c](#4c-studio-illustration-module-expérimental))
...
**Canvas chat (traits vectoriels, live)** — surface de dessin liée à la session (humain + agents). Humain : Sélect. (déplacer / supprimer / aligner / rotation), crayon, ligne,
courbe (points cliqués façon Bézier), silhouette (`path`), rectangle, ellipse, étiquettes texte (clic + saisie inline, rotation) ; calques nommés (masquer / verrouiller / opacité) ; grille et aimant 0.01 ; export PNG, SVG ou JSON. Agents : module WASM `canvas.*` (`stroke` /
`path` / `rect` / `ellipse` / `text` / `move` / `delete` / `align` / `rotate` / `layer_*` / `get` / `export`). Un validateur vectoriel global CPU-only contrôle la géométrie et la topologie toutes les trois mutations et avant la fin ; la vision reste facultative. Le seau (`canvas.fill`) n’est pas dans la barre humaine — flood-fill PNG seulement. Document : `var/sessions/<id>/canvas.json`. Distinct du studio Image et de `/image`.
...
| **Fenêtre d’édition** wgpu (`scene3d`) | Géométrie MeshBox/MeshAsset et couleur de base approximatives, avec hiérarchie GLB — **pas** rendu PBR/NPR complet |
...
| Assets | Import GLB natif et trois modèles Poly Haven CC0 téléchargés à la demande, avec provenance, licence et dimensions ; les assets déjà téléchargés ou importés se glissent dans la vue 3D ; marketplace général toujours désactivé |

**Ne pas lire ceci comme le MVP §140–§145 complet.** Manquent notamment une recherche étendue d’assets pour la composition, la QA beauté Blender dans le zip et un marketplace public. Blender reste un backend beauté derrière une frontière process — jamais l’éditeur de projet.
...
| `harness.run` | Spawn opt-in allowlisté des CLI locaux `codex` / `claude` / `grok` (prompt seulement, act-gate même en chat autonome) — détail [harness.md](harness.md) |
| Backend harness externe | Agents → Avancé → **Runtime** = Codex / Claude / Grok : les workers de tâche exécutent des tours CLI sous `aos-agent-worker` ; **Steer** continue la session (`exec resume` / `-c`), **Pause** annule le tour en cours, **Kill** arrête l’arbre worker — [harness.md](harness.md) |
