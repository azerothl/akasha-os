...
5. Quand un sous-agent termine, le runtime injecte `[child-done]` chez le parent. `agent.await` est optionnel ; intègre dès que le résultat arrive. Ne reste séquentiel que si un nœud dépend vraiment d'un autre.
