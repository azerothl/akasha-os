...
| Mémoire | `mem.*` + relations E6 (`similar` / `updates` / `supersedes`) | JSONL épisodique + `relations.jsonl` ; emprunt conceptuel sibling |
...
1. Exporter un JSON Schema / CBOR IDL des intents mémoire + secrets.
