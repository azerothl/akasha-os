...
E* number for these host tracks. Headless `serve` + local control plane
(P21.2–P21.3); agent intake still ahead (P21.4).

| # | Item | Status |
|---|------|--------|
| P21 / #403 | **`aos-serverd`** — ADR 0012 + lifecycle + headless serve + watchdogs + local `status`/`restart`/`stop` (P21.0–P21.3); intake API still ahead | **in progress** |
