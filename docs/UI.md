...
**Chat canvas** is a shared vector drawing surface on the same session (not Image Studio / not diffusion). Toggle with the session bar **Salon** / **Canvas** toggles (or `/canvas`). Bare « draw » / « dessine » routes to **Create** (Image Studio / pixels); vector canvas is used when the Canvas toggle is open or the message says « sur le canvas » / « au trait ». Humans draw with pen / eraser / rect / ellipse; agents use `canvas.*` tools only while Canvas is open. Strokes appear live (optimistic human paint + ~200 ms poll for agent ops with short draw-in animation). Export PNG is a snapshot under `/downloads`, not `media.image.generate`.
