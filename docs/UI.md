...
**Chat canvas** is a shared vector drawing surface on the same session (not Image Studio / not diffusion). Toggle with **Activer le canvas** / **Enable canvas** (or `/canvas`). Humans draw with pen / eraser / rect / ellipse; agents use `canvas.*` tools. Strokes appear live (optimistic human paint + ~200 ms poll for agent ops with short draw-in animation). Export PNG is a snapshot under `/downloads`, not `media.image.generate`.
